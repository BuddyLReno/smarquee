...
function createScrollTitle(contentToInsert, container) {
  var scrollContent = `<span data-smarquee-scroll-wrapper>${contentToInsert}<span data-smarquee-scroll-title>${contentToInsert}</span></span>`;
...
function calculateAnimationValues(totalWidth, velocity, separatorMargin = 0) {
  var finalDistance = totalWidth / 2 + separatorMargin;
...
function buildStyle(id, time, distance) {
  var style = `
...
class smarquee_Smarquee {
  constructor(options = {}) {
    ...
    this.styleBlock = appendStyleBlock(buildStyle(this.id, 15, 0), this.id);
  }
